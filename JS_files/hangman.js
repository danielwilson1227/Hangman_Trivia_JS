...
function getWord (){
	...
	if (category == 'tv') {
			...
			diff == 'easy' ? (startGame.word = media.longShows[rand - 1].toUpperCase()) :
			diff == 'med' ? (startGame.word = media.medShows[rand - 1].toUpperCase()) :
			(startGame.word = media.shortShows[rand - 1].toUpperCase());
	}
	else if (category == 'movie') {

			diff == 'easy' ? (startGame.word = media.longMovies[rand - 1].toUpperCase()) :
			diff == 'med' ? (startGame.word = media.medMovies[rand - 1].toUpperCase()) :
...
let startGame = {
	...
	fetchSumm     : function (){
		let fetchedLink;
			...
			category ==
			'movie' ? (fetchedLink = `https://api.themoviedb.org/3/search/movie?api_key=740531471cf0f1058187c583604c0c74&query=${this
				.word}`) :
			...
			.then((res) => {
				...
				res.json().then((data) => {
					...
					let posterData = data.results[0].backdrop_path;
						...
						this.word === 'JEOPARDY' ? (this.posterSrc = '/images/jeopardy.jpg') :
...
let checker = {
	...
	wrongAnswer    : function (){
		...
		if (h3.innerText.match(/\w/gi).length === 7) {
			...
			h3.innerText.match(/\w/gi).length === 8 ? gamePlay.loseGame() :
			gamePlay.hang();
	}
};
	...
	winChecker    : function (){
		...
		if (count === startGame.word.length) {
			...
			refresher();
				...
				diff === 'easy' ? (totalScore += 5) :
				diff === 'med' ? (totalScore += 10) :
				(totalScore += 15);
			if (localStorage.getItem('avatar') !== null) {
				localStorage.setItem('score', JSON.stringify(totalScore));
			}
